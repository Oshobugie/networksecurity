...
class DataValidation:
    ...
    def is_numerical_columns_exist(self, dataframe: pd.DataFrame) -> bool:
        """
        Validates if all the numerical columns specified in the schema.yaml 
        exist in the given dataframe.
        """
        try:
            ...
            if numerical_columns_present:
                logging.info("All required numerical columns are present in the dataframe.")
            else:
                logging.info(f"Validation Failed. Missing numerical columns: {missing_columns}")
    ...
    def detect_dataset_drift(self, base_df, current_df, threshold=0.05) -> bool:
        """
        Detects dataset drift between the base and current datasets using the Kolmogrov-Smirnov test. 
        Returns a boolean indicating overall drift status.
        """
        try:
            status = True
            report = {}
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        try:
            ...
            if not status:
                error_message = f"Testing data does not have the required number of columns.\n"
                
            # FIX 6: Added your numerical column checks
            self.is_numerical_columns_exist(train_dataframe)
            self.is_numerical_columns_exist(test_dataframe)
            ...
            # Create the 'validated' directory before saving the CSVs
